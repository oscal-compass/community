...
| Secure Plugin Framework  | Each plugin (C2P-Go only) is run in a fully independent process (leveraging `hashicorp/go-plugin`). This isolation limits the blast radius in the event of a compromised or crashing plugin. Communication with the main application is done via mTLS-secured gRPC over TCP and an enforced API contract defined in `protobuf`.                                       |
