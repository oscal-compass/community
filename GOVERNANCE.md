...
**Open**: OSCAL Compass is open source. See [project guidelines](./CONTRIBUTING.md).
...
While technical decisions for a project reside with its maintainers, the Oversight Committee may be asked to help mediate cross-project disputes or resolve decisions that a project's maintainers are unable to reach a consensus on. In the unlikely event that no good faith mediated consensus can be achieved, the oversight committee may chose a resolution, which will be binding.
